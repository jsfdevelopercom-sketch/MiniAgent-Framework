...
/**
 * Direct Claude model entity for single-model and group-chat experiences.
 *
 * This bypasses recursive MiniAgent loops intentionally.
 */
public class SimpleClaudeAgent implements SimpleModelAgent {

    private static final String AGENT_NAME = "Claude";
    ...
    public SimpleClaudeAgent(
            ...
            String defaultModel) {
        if (client == null) {
            throw new IllegalArgumentException("ClaudeHttpClient cannot be null.");
        }
        ...
        this.defaultModel = defaultModel == null || defaultModel.isBlank()
                ? client.getConfig().getDefaultClaudeModel()
                : defaultModel.trim();
    }
    ...
    public StructuredResponse respond(GroupChatContext context, String apiKeyOverride, Double temperature) {
        String runId = "simple-claude-" + UUID.randomUUID();
        ...
        String sysPrompt = SimpleAgentSupport.buildSystemPrompt(
                AGENT_NAME,
                PROVIDER_NAME,
                "Nuanced, careful, empathetic, skeptical, and strong at edge cases, tradeoffs, and human factors.",
                safeContext);
        ...
        try {
            ...
            String rawOutput = client.executeTextCall(
                    ...
                    sysPrompt,
                    userPrompt,
                    SimpleAgentSupport.safeTemperature(temperature, 0.3));
            ...
            recordCost("simple-claude", model, userPrompt, response.getSummary());
